...
def test_research_brief_is_redacted_and_aggregated(tmp_path):
 registry=MemoryRegistry(); row,_=registry.register(ExperimentSpec("momentum","f",{},"v1","abc","e1","c1"))
...
def test_prepare_creates_detached_review_only_worktree(tmp_path):
 repo=repository(tmp_path); registry=MemoryRegistry()
...
def test_prompt_has_explicit_safety_boundaries(tmp_path):
 repo=repository(tmp_path); workflow=CodexImprovementWorkflow(MemoryRegistry(),
