...
<section class=panel style='margin-top:16px'><h2>Secondary compute server <span id=computeHealth></span></h2><div class=cards id=computeCards></div><h3>16-core activity</h3><div class=cores id=remoteCores></div><div class=computeCharts><div id=throughputChart class=computeChart></div><div id=queueChart class=computeChart></div></div><table><thead><tr><th>Worker</th><th>Experiment</th><th>Family</th><th>Stage</th><th>Elapsed</th></tr></thead><tbody id=remoteWorkers></tbody></table></section>
<script>
document.addEventListener('DOMContentLoaded',()=>{const panels=[...document.querySelectorAll('section.panel')],resources=panels.find(x=>x.querySelector('h2')?.textContent.trim()==='Server resources'),compute=panels.find(x=>x.querySelector('h2')?.textContent.includes('Secondary compute server'));if(resources&&compute)resources.insertAdjacentElement('afterend',compute)});
