...
async function inspect(id){const x=await json('/api/experiments/'+id),g=x.validation?.gates??{},wf=x.validation?.walk_forward,bs=x.validation?.bootstrap;detail.innerHTML=`<h3>#${x.id} ${esc(x.strategy_family)} <span class=${x.status}>${x.status}</span></h3><p><b>Parameters</b><br><code>${esc(JSON.stringify(x.parameters))}</code></p><p><b>Gates</b><br>${Object.entries(g).map(([k,v])=>`<span class=${v?'pass':'fail'}>${v?'✓':'✗'} ${esc(k)}</span>`).join(' · ')||'Pending'}</p><p><b>Robustness</b><br>Positive walk-forward folds: ${wf?n(100*wf.positive_fold_fraction,0)+'%':'not reached'} · Bootstrap loss probability: ${bs?n(100*bs.loss_probability,1)+'%':'not reached'} · Bootstrap P05 P&amp;L: ${bs?n(bs.p05_net_pnl):'—'}</p><p><b>Timeline</b><br>${x.events.map(e=>esc(e.occurred_at.slice(11,19))+' '+esc(e.event)).join(' → ')}</p>`;if(x.artifacts?.equity){const f=await json('/api/tournament/equity/'+id);Plotly.react('tchart',f.data,f.layout)}else Plotly.purge('tchart')}
